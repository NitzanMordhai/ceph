...
void ThreadMonitor::deregister_thread(pid_t thread_id, const std::string& module_name) {
  std::lock_guard<std::mutex> lock(monitored_threads_mutex);
  ...
  if (it != monitored_threads.end() &&
      it->second.py_module &&
      it->second.py_module->get_name() == module_name) {
    dout(20) << "Deregistering thread: '" << it->second.name << "' (TID: " << thread_id << ")" << dendl;
    monitored_threads.erase(it);
  } else {
    dout(20) << "Skipped deregister unknown thread (TID: " << thread_id << ")." << dendl;
  }
}
...
void ThreadMonitor::monitoring_loop() {
  ...
  while (running) {
    ...
    std::vector<ThreadCheckInfo> threads_to_check;
    {
      std::lock_guard<std::mutex> lock(monitored_threads_mutex);
      for (auto& [tid, Info_thread] : monitored_threads) {
        // Copy the basic thread info to process the slow OS checks later
        threads_to_check.push_back({tid, Info_thread.serve_thread_id, Info_thread.name});

        // Update perf counters directly
        if (Info_thread.py_module && Info_thread.py_module->perfcounter) {
          long long rss_bytes = process_rss_pages * m_page_size;
          long long prev_rss_bytes = Info_thread.last_snapshot.rss_pages * m_page_size;
          long long rss_change = rss_bytes - prev_rss_bytes;

          Info_thread.py_module->perfcounter->set(Info_thread.py_module->l_pym_mem_rss_current, rss_bytes);
    ...
    std::map<pid_t, std::string> threads_to_remove;
    std::vector<std::pair<pid_t, pid_t>> serve_threads_to_remove;

    for (const auto& t : threads_to_check) {
      if (t.tid && !process_main_thread(t.tid)) {
        threads_to_remove[t.tid] = t.name;
        continue;
      }
      if (t.serve_thread_id && !process_serve_thread(t.tid, t.serve_thread_id)) {
        serve_threads_to_remove.push_back({t.tid, t.serve_thread_id});
      }
    }

    for (const auto& [tid_to_remove, module_name] : threads_to_remove) {
      deregister_thread(tid_to_remove, module_name);
    }

    // Clear out dead serve threads. Just a quick lock to update the map.
    if (!serve_threads_to_remove.empty()) {
      std::lock_guard<std::mutex> lock(monitored_threads_mutex);
