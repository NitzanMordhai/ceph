...
from mgr.cherrypy_mgr import CherryPyMgr

class TestCherryPyMgr(unittest.TestCase):
    def setUp(self):
        CherryPyMgr._trees = {}
        if hasattr(cherrypy, '_mgr_engine_started'):
            delattr(cherrypy, '_mgr_engine_started')
    ...
    @mock.patch('mgr.cherrypy_mgr.ServerAdapter')
    @mock.patch('mgr.cherrypy_mgr.WSGIServer')
    def test_mount(self, mock_wsgi_server, mock_server_adapter):
        ...
        self.assertTrue(getattr(cherrypy, '_mgr_engine_started', False))
    ...
    def test_mount_engine_already_started(self, mock_wsgi_server, mock_server_adapter):
        cherrypy._mgr_engine_started = True
    ...
    @mock.patch('mgr.cherrypy_mgr.BuiltinSSLAdapter')
    @mock.patch('mgr.cherrypy_mgr.ServerAdapter')
    @mock.patch('mgr.cherrypy_mgr.WSGIServer')
